...
import { fileSelectorAsDataURL } from "asr-client-typescript-client-lib";
...
export const BrowserAudioDeviceAreaDeviceSelect = (props: AudioDeviceAreaDeviceSelectProps) => {
    ...
    const select = useMemo(() => {
        const onAudioInputChanged = (event) => {
            voiceChangerClientState.setAudioInput(event.target.value);
        };
        const onAudioOutputChanged = (event) => {
            voiceChangerClientState.setAudioOutput(event.target.value);
        };
        const onAudioMonitorChanged = (event) => {
            voiceChangerClientState.setAudioMonitor(event.target.value);
        };
        ...
        if (props.type == "Input") {
            ...
            if (inputType == "file") {
                ...
                value = "screen";
            } else if (typeof voiceChangerClientState.audioInput === "string") {
                value = voiceChangerClientState.audioInput;
            } else {
                value = "none";
            }
            return (
            ...
            const value = voiceChangerClientState.audioOutput;
            return (
                <select
                    value={value}
                    onChange={(event) => {
            ...
            const value = voiceChangerClientState.audioMonitor;
            return (
                <select
                    value={value}
                    onChange={(event) => {
        ...
        voiceChangerClientState.audioInput,
        voiceChangerClientState.audioOutput,
        voiceChangerClientState.audioMonitor,
    ]);
    ...
    const audioPlayerRow = useMemo(() => {
        ...
        const onFileLoadClicked = async () => {
            ...
            voiceChangerClientState.setAudioInput(dst.stream);
        ...
        const fileIconUrl = generateGetPathFunc("/assets/icons/folder.svg");
        return (
            <div className={configSubAreaRow}>
                <div className={configSubAreaInputMediaContainer}>
                    ...
                    <img className={configSubAreaInputMediaFileSelectIcon} src={fileIconUrl} onClick={onFileLoadClicked} />
                    <div
    ...
    useEffect(() => {
        const setEchobackAudioOutput = async () => {
            ...
            if (voiceChangerClientState.audioOutput != "none") {
                try {
                    await audio_echo.setSinkId(voiceChangerClientState.audioOutput);
                } catch (e) {
    ...
    }, [inputType, enableEchoback, voiceChangerClientState.audioOutput]);
    ...
    const audioCaptureRow = useMemo(() => {
        ...
        const onCaptureClicked = async () => {
            ...
            try {
                voiceChangerClientState.setAudioInput(displayMediaStream.current);
            } catch (e) {
                console.error(e);
            }
    ...
    const component = useMemo(() => {
        ...
        return (
            <>
                <div className={configSubAreaRow}>
                    <div className={BasicLabel({ width: "large", paddingLeft: "1rem" })}>{name}:</div>
